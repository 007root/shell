...
echo -e "ServerName: \033[32msanguo sanguo01 sanguo02 wow\033[0m"
...
if [ "$ser_name" == "sanguo02" ]; then
	remote_ip="192.168.1.139"
	...
	remote_run_port=`SSH "forever --no-colors list | grep ${ser_name}_" | awk '{print $6}' | awk -F"_" '{print $2}'`
	remote_run_uid=`SSH "forever --no-colors list | grep ${ser_name}_" | awk '{print $3}'`
	...
	do
		...
		scp -r  ./AccountService/* $remote_ip:$remote_dir/$dir &> /dev/null
	done
	...
	CheckService() {
		if [[ -n $stat_flag || -z $log_flag ]]; then
			scp $remote_ip:/root/.forever/$remote_up_uid.log $log_dir &> /dev/null
			SSH "forever stop $remote_up_uid"
			echo -e "\033[31m$ser_name Service update faild. Information $log_dir/$remote_up_uid.log \033[0m"
			break
		else
			SSH "sed -i 's/#server $remote_ip:$port/server $remote_ip:$port/' $nginx_config"
			SSH "sed -i 's/server $remote_ip:$remote_run_port/#server $remote_ip:$remote_run_port/' $nginx_config"
			SSH "/usr/local/nginx/sbin/nginx -s reload"
			sleep 2
	...
	StartService() {
		...
		SSH "cd $remote_dir/$port/ && forever start accountService ${ser_name}_$port" &>/dev/null
		sleep 1
	}
	...
	do
		...
		SSH "cd $remote_dir/$port/ && node accountService updatedb" &> /dev/null
		sleep 1
		# start service	
		StartService
		# check service
		echo -e "\033[32mCHECK SERVICE ...\033[0m"
		remote_up_uid=`SSH "forever --no-colors list | grep ${ser_name}_$port" | awk '{print $3}'`
		log_flag=`SSH "cat /root/.forever/${remote_up_uid}.log | grep -o '$remote_ip:$port'"`
		stat_flag=`SSH "forever --no-colors list | grep ${ser_name}_$port | grep -i stopped"`
		CheckService 
	done
	...
	do
		...
		CheckService() {
			if [[ -n $stat_flag || -z $log_flag ]]; then
				cp /root/.forever/$remote_up_uid.log $log_dir &> /dev/null
